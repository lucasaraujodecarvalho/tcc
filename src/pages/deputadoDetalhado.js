...
export default class DeputadoDetalhado extends Component {
  ...
  loadDeputadosGastos = async navigation => {
    const id = navigation.getParam('id');
    let deputado = await api.get(
      `/deputados/${id}/despesas?ano=2019&itens=100`,
    );
    let totalGastos = 0;
    ...
    for (let gasto of deputado.data.dados) {
      ...
      if (
        gasto.tipoDespesa == 'LOCAÇÃO OU FRETAMENTO DE VEÍCULOS AUTOMOTORES'
      ) {
        totalGastosManutencao += gasto.valorLiquido;
        tipoDespesasManutencao = gasto.tipoDespesa;
      }
      if (
        gasto.tipoDespesa == 'CONSULTORIAS, PESQUISAS E TRABALHOS TÉCNICOS.'
      ) {
  ...
  favoritar = async novoDeputadoPraSerFavorito => {
  ...
  render() {
    return (
      <ScrollView style={styles.productContainer}>
        ...
        <TouchableOpacity onPress={() => this.favoritar(this.state.dados)}>
        ...
        <View style={styles.deputadoContainer}>
          ...
          <Text style={styles.separandoItens}>
            Nascimento:{' '}
            {moment(this.state.dados.dataNascimento).format('DD/MM/YYYY')}{' '}
            {this.state.dados.municipioNascimento},{' '}
            {this.state.dados.ufNascimento}
          </Text>

          <Text style={styles.palavraNegrito} size={20}>
            CONTATO:
          </Text>
          <Text>
            <Icon name="phone" size={14} /> {this.state.gabinete.telefone}
          </Text>
          <Text style={styles.separandoItens}>
            <Icon name="envelope-o" size={14} /> {this.state.gabinete.email}
          </Text>

          <Text style={styles.palavraNegrito} size={20}>
            GASTOS:
          </Text>
          <Text style={styles.palavraNegrito}>
            <Icon name="usd" size={14} /> Total de Gastos 2019-2022:{' '}
          </Text>
          <TextMask type={'money'} value={this.state.valorTotal}>
            R${' '}
          </TextMask>
          <Text style={styles.palavraNegrito}>Principais Despesas:</Text>

          {this.state.tipoDespesaManutencao ? (
            <Text>
              Manutenção de Escritório de apoio á atividade parlamentar:{' '}
            </Text>
          ) : null}
          {this.state.tipoDespesaManutencao ? (
            <TextMask type={'money'} value={this.state.valorTotalManutencao} />
          ) : null}

          {this.state.tipoDespesaCombustivel ? (
            <Text>Divulgação da atividade parlamentar: </Text>
          ) : null}
          {this.state.tipoDespesaCombustivel ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasCombustivel}
            />
          ) : null}

          {this.state.tipoDespesaDivulgacao ? <Text>Telefonia: </Text> : null}
          {this.state.tipoDespesaDivulgacao ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasDivulgacao}
            />
          ) : null}

          {this.state.tipoDespesaTelefonia ? (
            <Text>Serviços Postais: </Text>
          ) : null}
          {this.state.tipoDespesaTelefonia ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasTelefonia}
            />
          ) : null}

          {this.state.tipoDespesaServicosPostais ? (
            <Text>Fornecimento de alimentação do parlamentar: </Text>
          ) : null}
          {this.state.tipoDespesaServicosPostais ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasServicosPostais}>
              {this.state.tipoDespesaServicosPostais}
            </TextMask>
          ) : null}

          {this.state.tipoDespesaFornecimento ? (
            <Text>Emissão bilhete aéreo: </Text>
          ) : null}
          {this.state.tipoDespesaFornecimento ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasFornecimento}
            />
          ) : null}

          {this.state.tipoDespesaEmissao ? (
            <Text>Hospedagem, Exceto do parlamentar no Distrito Federal: </Text>
          ) : null}
          {this.state.tipoDespesaEmissao ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasEmissao}
            />
          ) : null}

          {this.state.tipoDespesaHospedagem ? (
            <Text>Serviço de táxi, pedágio e estacionamento: </Text>
          ) : null}
          {this.state.tipoDespesaHospedagem ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasHospedagem}
            />
          ) : null}

          {this.state.tipoDespesaServico ? (
            <Text>Locação ou fretamento de veículos automotores: </Text>
          ) : null}
          {this.state.tipoDespesaServico ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasServico}
            />
          ) : null}

          {this.state.tipoDespesaConsultorias ? (
            <Text>Consultorias, pesquisas e trabalhos técnicos: </Text>
          ) : null}
          {this.state.tipoDespesaConsultorias ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasConsultorias}
            />
          ) : null}

          {this.state.tipoDespesaPassagens ? (
            <Text>Passagens Aéreas: </Text>
          ) : null}
          {this.state.tipoDespesaPassagens ? (
            <TextMask
              type={'money'}
              value={this.state.valorTotalDespesasPassagens}
            />
          ) : null}
        </View>
      </ScrollView>
